...
class HomeTableViewController: UITableViewController, UISearchBarDelegate, NSFetchedResultsControllerDelegate {
    ...
    func recuperaAluno(){
        ...
        let ordenaAlfabetica = NSSortDescriptor(key: "nome", ascending : true)
        pesquisaAluno.sortDescriptors = [ordenaAlfabetica]
        ...
        do {try gerenciadorResultados?.performFetch()}catch{print(error.localizedDescription)}
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
           if segue.identifier == "editarAluno"{
    ...
    @objc func abrirActionSheet(_ longPress : UILongPressGestureRecognizer){
        if longPress.state == .began{
            ...
            let menu = MenuOpcoesAlunos().configuraMenuDeOpcoesDoAluno { (opcao) in
                switch opcao{
                case .sms:
                    guard let componenteSMS =  self.mensagem.configuraSms(alunoSelecionado) else {return }
                    componenteSMS.messageComposeDelegate = self.mensagem
                    self.present(componenteSMS, animated: true, completion: nil)
                    break
                case .ligacao:
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        guard let contadorAlunos = gerenciadorResultados?.fetchedObjects?.count else {return 0 }
